...
export default function QuakeDirasakanScreen({ navigation }) {
  const [quakeDirasakan, setQuakeDirasakan] = useState([]);

  useEffect(() => {
    fetch("https://data.bmkg.go.id/DataMKG/TEWS/gempadirasakan.json")
      .then((res) => res.json())
      .then((data) => setQuakeDirasakan(data.Infogempa.gempa));
  }, []);
